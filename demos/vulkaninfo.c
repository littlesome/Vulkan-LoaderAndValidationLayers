...
static void AppDevDumpFormatProps(const struct AppDev *dev, VkFormat fmt, bool *first_in_list, FILE *out) {
...
static void AppDevDump(const struct AppDev *dev, FILE *out) {
    ...
    for (fmt = 0; fmt < VK_FORMAT_RANGE_SIZE; ++fmt) {
        AppDevDumpFormatProps(dev, fmt, &first_in_list, out);
    }
    if (html_output) fprintf(out, "\t\t\t\t\t</details>\n");
...
static void AppDumpExtensions(const char *indent, const char *layer_name, const uint32_t extension_count,
    ...
    bool device_type = strcmp(layer_name, "Device") == 0;
    if (device_type && json_output) {
        printf(",\n");
...
//--XLIB--
    ...
    fflush(stdout);
#ifdef _WIN32
    if (ConsoleIsExclusive() && console_output) Sleep(INFINITE);
#endif
